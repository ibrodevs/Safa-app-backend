...
def test_group_selection_and_move_logic():
    ...
    assert "const target = featureAtLatLng(event?.latLng, id) || id;" in javascript
    # Среди соседних контейнеров побеждает именно overlay, по которому кликнули,
    # а не первый контейнер того же типа в state.items.
    assert "function featureAtLatLng(latLng, preferredId = null)" in javascript
    assert "const preferredKind = (preferred?.feature.properties || {}).kind;" in javascript
    assert "if (preferredKind === 'passage') return preferredId;" in javascript
    ...
    assert "moveEditingSelectionToGroup(selectedId);" in javascript
    ...
    # Даже если Google Maps отдаст первый клик базовой карте, а не маленькому
    # overlay, координатный fallback обязан выбрать контейнер в тот же кадр.
    assert "state.lastOverlayPickAt = Date.now();" in javascript
    ...
    assert "const groupMovable = highlighted && !state.pickMode;" in javascript
    assert "overlay.setDraggable(groupMovable);" in javascript
    assert "draggable: groupMovable," in javascript
