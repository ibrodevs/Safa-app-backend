...
@override_settings(
    ...
    STATIC_OTP_CODE="",
    ALLOW_STATIC_OTP_IN_PRODUCTION=False,
    CHATFLOW_TOKEN="",
)
...
@override_settings(
    DEBUG=False,
    ENABLE_DEBUG_OTP_ENDPOINT=False,
    STATIC_OTP={},
    STATIC_OTP_CODE="1111",
    ALLOW_STATIC_OTP_IN_PRODUCTION=True,
    CHATFLOW_TOKEN="",
)
def test_global_static_otp_allows_any_phone_in_production():
    phone = "996700123460"
    get_user_model().objects.create_user(
        phone_number=phone,
        password="secret123",
        first_name="Static OTP User",
    )
    client = APIClient()
    ...
    assert request_response.status_code == 200
    assert request_response.data["detail"] == "sent_static"
